...
data_1<-read_csv(here("Data", "297716", "GRBGBWQ.csv"), skip = 2)#skipping first two rows 

source(here("Scripts", "theme_matt.R"))
...
b<-ggplot(phys_data, aes(x = same_year(Date),y = pH, group = as.factor(year(Date)), 
  ...
  geom_line(aes(linetype = as.factor(year(Date))), size = 1.5)+
...
```{r Analyses for figure 4, echo = FALSE, results = "hide"}
...
```{r Figure 5, fig.cap = "Figure 5. Back transformed regression model of D-hinge oyster larvae and temperature (°C) with corresponding trendline and 95% confidence intervals shown in gray.", echo = FALSE, fig.height=6, warning = FALSE}
#plot
new_pred %>% 
